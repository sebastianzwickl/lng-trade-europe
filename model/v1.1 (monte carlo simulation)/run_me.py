...
import monte_carlo_simulation_plots as mcp
...
# MONTE CARLO SIMULATION (CHOICE = 6)
_monte_carlo_average = []
_monte_carlo_marginal = []
_monte_carlo_ccs = []
n = 0
_iteration = 10000
...
while n < _iteration:
    ...
    if _choice == 6:
        for ex, im in des_dict.keys():
            if ex in ["Qatar", "Oman", "Other ME"]:
                des_dict[ex, im] *= np.random.uniform(1, 1.25001)
            if ex in ["Algeria", "Other Africa", "Nigeria"]:
                des_dict[ex, im] *= np.random.uniform(1, 5.00001)
        ...
        for ex, im in des_dict.keys():
            ...
            for _index, _row in _read_pan_routes.iterrows():
                if (ex == _row.Exporter) and (im == _row.Importer):
                    _flag = 1
                    des_dict[ex, im] *= np.random.uniform(1, 1.33001)
                else:
                    pass
            if _flag == 0:
                des_dict[ex, im] *= np.random.uniform(1, 1.15001)
            else:
                pass
    else:
        pass
    ...
    _ccs_cost *= np.random.uniform(0.75, 1.25001)
    ...
    _value *= np.random.uniform(0.75, 1.15001)
    ...
    def ccs_technology_utilization(m):
        return (
            sum(m.var_q_dom_europe[i] for i in m.set_importer_europe) <= m.par_max_EDP
        )
    ...
    def objective_function(m):
        _market = sum(
            m.var_cost_market_clearing[importer] for importer in m.set_importer
        )
    ...
    _cases = [0, 1, 2]
    _random_combination = np.random.choice(
        _cases, np.random.randint(1, len(_cases) + 1), replace=False
    )
    # print(_random_combination)
    if 1 in _random_combination:
        #  (1) ... Diverse (strong diversification of exporters in Europe)
    ...
    if (not os.path.exists(result_dir)) and (n == (_iteration-1)):
    ...
    _monte_carlo_average.append(_v_av)
    _monte_carlo_marginal.append(_v_mg)

    _column = "Import 2040 (" + _SCENARIO + ") [MMBtu]"
    total_european_lng_demand = np.around(
        sum(_demand.loc[_demand.Country.isin(_importers_europe)][_column]), 1
    )

    _monte_carlo_ccs.append(
        model.var_cost_edp() / model.par_EDP_cost() / total_european_lng_demand
    )

    n += 1

data = [_monte_carlo_average, _monte_carlo_marginal, _monte_carlo_ccs]
mcp.run_me(data, result_dir)
